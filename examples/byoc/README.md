...
The last path segment must match your registered capability name
(``text-reversal`` in ``register_capability.py``); the orchestrator forwards
``/process/request/<segment>`` to ``{worker url}/<segment>``.
...
curl -X POST "$ORCHESTRATOR_URL/capability/register" \
  ...
  -d "{\"name\":\"text-reversal\",\"url\":\"$WORKER_URL\",\"capacity\":1,\"price_per_unit\":1,\"price_scaling\":1,\"currency\":\"wei\"}"
```
