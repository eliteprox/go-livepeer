...
func CreateOrchNode(uri *url.URL, router_uri *url.URL) OrchNode {
	return OrchNode{uri: *uri, routerUri: *router_uri}
}
	...
	for {
		...
		case <-totCh:
			...
			if totCtr >= len(r.orchNodes) && r.roundRobin {
				fastest := int64(4000) //start with 4000ms (4 seconds), all should be faster than this
				fastestIdx := 0
				for idx, resp := range(responses) {
					if resp.RespTime < fastest{
						fastest = resp.RespTime
						fastestIdx = idx
					}
				}
				r.SetClosestOrchestrator(client_ip, responses[fastestIdx])
				glog.Infof("%v  received all responses, sending orchestrator info for %v", client_ip, responses[fastestIdx].OrchUri.String())
				return r.GetOrchestratorInfo(ctx, req, responses[fastestIdx].OrchUri)
			}
		case <-lctx.Done():
			//when the context time limit is complete, return the closest orchestrator so far
			closestOrchWithResp, err := r.GetClosestOrchestrator(client_ip)
			if err == nil {
				go r.updateRouters(client_ip, &closestOrchWithResp)
				return r.GetOrchestratorInfo(ctx, req, closestOrchWithResp.OrchUri)
			} else {
				return nil, errNoOrchestrators
			}
		}
...
func (r *LatencyRouter) GetOrchNodeInfo(orch_uri url.URL) *net.OrchestratorInfo {
	r.bmu.RLock()
	defer r.bmu.RUnlock()
	orch_node, orch_node_exists := r.orchNodes[orch_uri]
	if orch_node_exists {
		return &orch_node.orchInfo
	} else {
		return nil
	}
}

func (r *LatencyRouter) SetOrchNodeInfo(orch_uri url.URL, orch_info *net.OrchestratorInfo) {
	r.bmu.Lock()
	defer r.bmu.Unlock()

	//clone the orch info to OrchNode to cache and remove reference to pointer
	glog.Infof("orch info cached for %v at %v", orch_uri.String(), time.Now())
	node, ok := r.orchNodes[orch_uri]
	if ok {
		node.orchInfo = *orch_info
		node.updatedAt = time.Now()
	}
	r.orchNodes[orch_uri] = node
}

func (r *LatencyRouter) GetOrchestratorInfo(ctx context.Context, req *net.OrchestratorRequest, orch_uri url.URL) (*net.OrchestratorInfo, error) {
	...
	r.SetOrchNodeInfo(orch_uri, info)
	return info, nil
}
